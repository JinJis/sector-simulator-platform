...
| **M34** | LLM provider swap (Claude → Gemini). Replace Anthropic SDK with `google-genai` (Python) + `@google/genai` (TS) across the entire platform. Single API key (`GEMINI_API_KEY`). Tier mapping: `opus` → `gemini-3.5-flash` (post-2026-05 swap from `gemini-3.1-pro-preview`, which Vertex AI doesn't expose), `sonnet` → `gemini-3-flash-preview`, `haiku` → `gemini-3.1-flash-lite`. `LLMClient.call()` interface preserved — no workflow changes. Cost meter rebuilt with Gemini 3.x pricing ($0.50/$3 Flash for opus + sonnet, $0.25/$1.50 Flash-Lite for haiku per 1M tokens; cache-read 0.25×). Expected per-user monthly LLM bill drops ~80% vs Claude. 20 agent-tools tests, 56 agent-orchestration tests, 34 agent-evals — all green. |
...
| **M35** | Dual-provider LLM (Claude opus back; Gemini stays for sonnet/haiku). `LLMClient` now wraps BOTH `anthropic[vertex]` (`AnthropicVertex`, region `global`) and `google-genai` (`genai.Client(vertexai=True, location="global", credentials=...)`) behind the same `.call(tier=...)` surface. Tier mapping: `opus` → `claude-opus-4-7`, `sonnet` → `gemini-3.5-flash`, `haiku` → `gemini-3.5-flash-lite`. Same SA JSON authenticates both providers; `project_id` is auto-extracted from the JSON (no longer requires `GOOGLE_CLOUD_PROJECT`). `GOOGLE_CLOUD_LOCATION` default flipped to `global`. Structured output: Gemini keeps native `response_schema`; Claude uses the tool-use trick (forced `tool_choice` + Pydantic JSON schema as `tools[0].input_schema`). `adaptive_thinking=True` now wires to Claude's extended thinking (`budget_tokens` = min(max_tokens/2, 8192)) on the opus path. TS `prediction.analyzeRationale` also moved to `gemini-3.5-flash` + `location="global"`. cost.py adds Claude pricing back ($15/$75 with 0.1× cache read, 1.25× cache write). Adds `LLMClient.clone(cost_meter=...)` to replace the `_client`-poking pattern in `workflows.py` (lets per-workflow meters work with dual clients). Tests: 30 agent-tools (15 Gemini, 6 Anthropic, 4 default-client builder, 5 cost), agent-orchestration conftest gets a unified `FakeGenAI` exposing both `models.generate_content` and `messages.create` with a shared `requests` list. |
...
| `GOOGLE_GENAI_USE_VERTEXAI` | `true` | M34b/M35: switch to Vertex AI auth for both Gemini + Claude. Set to `false` for the Gemini-only API-key fallback (opus tier disabled) |
| `GOOGLE_CLOUD_PROJECT` | (unset) | GCP project ID. Optional — when blank, read from the SA JSON's `project_id` field |
| `GOOGLE_CLOUD_LOCATION` | `global` | Vertex AI region. `global` is the multi-region endpoint hosting Gemini 3.5-flash family + Claude opus 4.7 |
| `GOOGLE_APPLICATION_CREDENTIALS` | `/secrets/vertex-ai-sa.json` | In-container path to the service-account JSON; bind-mounted from `infra/secrets/`. Same SA authenticates Gemini + Claude |
| `GEMINI_API_KEY` | (unset) | Dev-only fallback when Vertex AI is unavailable. Opus-tier (Claude) calls fail in this mode |
