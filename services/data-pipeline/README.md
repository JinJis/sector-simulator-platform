...
*   **Execution Flow**:
    ...
    2. Issues queries to RSS/news 어댑터 and collects raw signals.
    ...
    4. If unique, dispatches to the **SignalExtractor agent (Gemini Haiku tier)** to extract signed score deltas (`delta_*` $\in [-10, +10]$), highlight markers (`is_highlight`), and match `Actor` mentions.
...
*   **Execution Flow**:
    ...
    3. Invokes the **ScoreUpdater agent (Gemini Sonnet tier)** with the baseline scores and the cumulative signals list.
