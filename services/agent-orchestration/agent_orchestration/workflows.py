"""Concrete agent workflows — DecompositionWorkflow + 16 siblings.

The :class:`Workflow` protocol and :class:`WorkflowRunner` scheduler
live in :mod:`agent_orchestration.runner` and are re-exported below so
existing ``from agent_orchestration.workflows import WorkflowRunner``
...
from agent_orchestration.runner import (
    InputT,
    OutputT,
    Workflow,
    WorkflowRunner,
...
# Re-export so `from agent_orchestration.workflows import WorkflowRunner`
# (and `Workflow`, `InputT`, `OutputT`) keeps working after the runner
# extraction. New code should import from `agent_orchestration.runner`
# directly.
__all__ = [
    "InputT",
    "OutputT",
